...
def generate_article_content(topic_info):
    ...
    model = genai.GenerativeModel(MODEL_NAME)
    prompt = f"""
    あなたはセルフケアブランド「fucuu」の専属ライターであり、東洋医学・自律神経の専門家です。
    以下のテーマに基づき、読者の深い悩みを解決するSEOブログ記事を執筆してください。
    ...
    【超重要：執筆ルール】
    1. 目的とスタンス: 読者の「悩み解決」を100%最優先にしてください。医学的・科学的に正確な情報を、素人にも極めてわかりやすく解説すること。間違った民間療法や不確かな情報は絶対に書かないでください。
    2. 構成: 
       - 導入（悩みに深く共感する）
       - 原因のロジカルな解説（なぜ不調が起きるのか）
       - 具体的な解決策3選（明日からすぐ無料で実践できる具体的なアクション。例：ツボ押し、ストレッチなど）
       - まとめとfucuuからの提案
    3. 販売への自然な導線: 記事の本文中は一切売り込みをせず「信頼のおける専門家」に徹してください。最後の「まとめ」のセクションでのみ、「忙しくてセルフケアの時間が取れない方へ」という文脈で、fucuuの100%生薬入浴剤や国産アロマが「究極の時短セルフケア」として役立つことを上品に提案してください。これが最終的な販売拡大（コンバージョン）に繋がります。
    4. 文字数: 1500〜2000文字程度。
    5. 特殊タグの活用:
